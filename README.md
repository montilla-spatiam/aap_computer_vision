...
### Receiving images
`aap_receive_cv.py` sets up the receiver, and forwards back labels to image sender.

````
$ python3 aap_receive_cv.py --socket <path_to_socket_file> --agentid sink_cv
````

### Sending an image
`aap_send_cv.py` sends an image, and waits for a response containing image labels.
